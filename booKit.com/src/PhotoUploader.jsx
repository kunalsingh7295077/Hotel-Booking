...
export default function PhotoUploader({addedPhotos , onChange}){
    ...
    async function addPhototByLink(ev){
        ev.preventDefault();
    ...
    return(
        <>
            {/* link for uploading photo */}
            <div className="flex gap-2">
                <input type="text" 
                    value={photoLink} onChange={ev => setPhotoLink(ev.target.value)}
                    placeholder="Add using a link.....jpg"/>
                <button onClick={addPhototByLink} className="bg-gray-200 px-4 rounded-2xl">Add&nbsp;photo</button>
            </div>


            {/* upload button */}
            <div className="mt-2 grid gap-2 grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
                {/* displaying photo using map */}
                ...
                <label className="h-32 cursor-pointer flex justify-center gap-1 items-center border bg-transparent rounded-2xl p-5 text-2xl text-gray-600">
